 ...
 * (4)アンフィット
 */
public class MyUnfitAdapterCom extends CommonAdapter<ResponseCheck.ItemInfo> {
	 ...
	 *            一覧データ
	 */
	public MyUnfitAdapterCom(Context context, int resource, List<ResponseCheck.ItemInfo> objects, String screenId) {
...
*/
			...
			if (itemInfo.isQuote()) {
		        ...
		        //出荷日数の表示
				{
			        //出荷日数
			        //出荷日数(daysToShip)＋(＋出荷区分(shipType)＋)
					if (android.text.TextUtils.isEmpty(((ResponseCheckOrder.ItemInfo)itemInfo).shipDate)) {
	
						str = getResourceString(R.string.label_hyphen);   //ハイフン化
	
					} else {
	
						str = ((ResponseCheckOrder.ItemInfo)itemInfo).shipDate;
					}
...
//				setIncludeItemText(childView.findViewById(R.id.expressType), getResourceString(R.string.unfit_type_dialog_list_emarg_main), str, false);
				//課題 #4520
		        if (android.text.TextUtils.isEmpty(str)) {
					str = getResourceString(R.string.label_hyphen);   //ハイフン化
				}
				setIncludeItemText(childView.findViewById(R.id.expressType), getResourceString(R.string.unfit_type_dialog_list_emarg_main) + str, String.valueOf(Character.toChars(0x200B)), false);
			}
