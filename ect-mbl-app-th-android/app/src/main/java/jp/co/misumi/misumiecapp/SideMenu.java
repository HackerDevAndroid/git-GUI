...
import jp.co.misumi.misumiecapp.fragment.LanguageSettingFragment;
import jp.co.misumi.misumiecapp.fragment.MyPartsListFragment;
import jp.co.misumi.misumiecapp.fragment.OrderListFragment;
import jp.co.misumi.misumiecapp.fragment.SmsSettingFragment;
...
public class SideMenu implements View.OnClickListener {
    ...
    public SideMenu(Activity activity) {
        ...
        mDrawerView.findViewById(R.id.frameLanguageSetting).setOnClickListener(this);
        mDrawerView.findViewById(R.id.frameSmsSetting).setOnClickListener(this);
...
//        MISUMI_MOBILE_APP-583 【中国版】メニューの営業日・営業時間を削除したい
        ...
        // SPv1_40_言語設定_画面定義書_v07
        if (SubsidiaryCode.isThailand()) {
...
//            AppLog.d("AppState Changed(SlideMenu) = " + notice.event);
    ...
    // SPv1_40_言語設定_画面定義書_v07
    private void showLanguageSetting(boolean isShow) {
        if (isShow) {
            mDrawerView.findViewById(R.id.frameLanguageSetting).setVisibility(View.VISIBLE);
        } else {
            mDrawerView.findViewById(R.id.frameLanguageSetting).setVisibility(View.GONE);
        }
    }

    // SPv1_41_SMS設定_画面定義書_v07
    private void showSmsSetting(boolean isShow) {
        if (isShow) {
            mDrawerView.findViewById(R.id.frameSmsSetting).setVisibility(View.VISIBLE);
        } else {
            mDrawerView.findViewById(R.id.frameSmsSetting).setVisibility(View.GONE);
        }
    }
...
//                GoogleAnalytics.sendAction(mainActivity.getTracker(),null,GoogleAnalytics.CATEGORY_REGISTER,"-");
            ...
            case R.id.frameDebug:
                ...
                break;
            case R.id.frameLanguageSetting:
                if (!(mFragmentController.getCurrentFragment() instanceof LanguageSettingFragment)) {
                    mFragmentController.stackFragment(new LanguageSettingFragment(), FragmentController.ANIMATION_SLIDE_IN);
                }
                //Todo: Declare GoogleAnalytics value
//                GoogleAnalytics.sendAction(mainActivity.getTracker(), GoogleAnalytics.CATEGORY_MENU, GoogleAnalytics.lable_phoneInfo);
                break;
            case R.id.frameSmsSetting:
                if (!(mFragmentController.getCurrentFragment() instanceof SmsSettingFragment)) {
                    mFragmentController.stackFragment(new SmsSettingFragment(), FragmentController.ANIMATION_SLIDE_IN);
                }
                //Todo: Declare GoogleAnalytics value
//                GoogleAnalytics.sendAction(mainActivity.getTracker(), GoogleAnalytics.CATEGORY_MENU, GoogleAnalytics.lable_phoneInfo);
                break;
